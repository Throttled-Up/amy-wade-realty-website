...
export default function Nav({ config }) {
  const { business_name, phone, hubs = [] } = config;
          ...
          {navItems.map(item => (
            <li key={item.href}>
              <Link
                ...
                onMouseEnter={e => (e.currentTarget.style.color = 'var(--color-nav-text-hover)')}
                onMouseLeave={e => (e.currentTarget.style.color = 'var(--color-nav-text)')}
              >
                {item.label}
              </Link>
            </li>
      ...
      {menuOpen && (
        <div
          style={{
            ...
            borderTop:  '1px solid var(--color-nav-mobile-border)',
            padding:    '16px 0 24px',
          }}
        >
          <ul className="container" style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {navItems.map(item => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  ...
                  style={{
                    display:      'block',
                    color:        'var(--color-nav-mobile-link)',
                    fontSize:     '1rem',
                    fontWeight:   500,
                    padding:      '12px 0',
                    borderBottom: '1px solid var(--color-nav-mobile-divider)',
                  }}
                >
                  {item.label}
                </Link>
